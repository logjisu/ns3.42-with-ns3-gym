...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    uint32_t m_routingTableChangeCount;

    uint32_t GetRecentRoutingTableChanges() const;
    void LogRoutingTableChanges() const;
    void SetHelloInterval(Time interval); // Action 적용
    void SetTcInterval(Time interval);    // Action 적용
    uint32_t GetQValue() const;           // State 반환

  private:
    mutable uint32_t m_qValue;    // QValue 상태
    std::deque<Time> m_routingTableChangeTimes;
