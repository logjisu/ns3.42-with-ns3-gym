...
RoutingProtocol::RoutingTableComputation()
{
    ...
    uint32_t oldSize = m_table.size();
    // 1. All the entries from the routing table are removed.
    std::map<Ipv4Address, RoutingTableEntry> oldTable = m_table; // 기존 테이블 복사
    Clear();
    ...
    if (m_table.size() != oldSize) {
        m_routingTableChangeTimes.push_back(Simulator::Now());

        // 2초 이상 지난 타임스태프 제거
        while (!m_routingTableChangeTimes.empty() &&
            (Simulator::Now() - m_routingTableChangeTimes.front()).GetSeconds() > 5.0) {
        m_routingTableChangeTimes.pop_front();
        }

        LogRoutingTableChanges();
    }
...
void RoutingProtocol::LogRoutingTableChanges() const
{
    NS_LOG_INFO("Node " << m_ipv4->GetObject<Node>()->GetId()
                        << " Routing Table Changes in last 2 seconds: "
                        << GetRecentRoutingTableChanges() << "\tQValue : " << GetQValue());
}


uint32_t RoutingProtocol::GetRecentRoutingTableChanges() const
{
    return m_routingTableChangeTimes.size();
}

void RoutingProtocol::SetHelloInterval(Time interval) {
    m_helloInterval = interval;
    NS_LOG_INFO("HelloInterval updated to: " << m_helloInterval);
}

void RoutingProtocol::SetTcInterval(Time interval) {
    m_tcInterval = interval;
    NS_LOG_INFO("TcInterval updated to: " << m_tcInterval);
}

uint32_t RoutingProtocol::GetQValue() const
{   
    uint32_t size = m_routingTableChangeTimes.size();
    if (size <= 3) {
        m_qValue = 1;
    } else if (size <= 6) {
        m_qValue = 2;
